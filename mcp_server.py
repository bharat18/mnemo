...
async def list_tools() -> list[types.Tool]:
    return [
        ...
        types.Tool(
            name="watch_video",
            description=(
                "Extract frames from a YouTube video or local video file and return them "
                "as images so Claude can SEE the video using vision capabilities. "
                "Works with YouTube URLs (youtu.be / youtube.com) and local file paths (.mp4 etc). "
            ...
            inputSchema={
                ...
                "properties": {
                    "url": {
                        "type": "string",
                        "description": (
                            "YouTube URL (https://youtu.be/... or https://youtube.com/watch?v=...) "
                            "OR absolute path to a local video file."
                        ),
                    },
                    "interval_sec": {
                        "type": "integer",
                        "description": "Extract one frame every N seconds (default: 8). Use 4-6 for demos, 10-15 for long videos.",
                        "default": 8,
                    },
                    "max_frames": {
                        "type": "integer",
                        "description": "Max frames to return (default: 20, max: 40).",
                        "default": 20,
                    },
                },
                "required": ["url"],
        ...
        types.Tool(
            ...
            description=(
                "Get the transcript / captions from a YouTube video as timestamped text. "
                "Much faster than watch_video when you only need spoken content. "
                "Uses youtube-transcript-api if installed, otherwise falls back to yt-dlp auto-subs. "
...
{existing_yaml}"""
    ...
    # ── watch_video ───────────────────────────────────────────────────────────
        ...
        interval_sec = max(1, int(arguments.get("interval_sec", 8)))
        max_frames   = min(40, max(1, int(arguments.get("max_frames", 20))))

        if not url:
            return [types.TextContent(type="text", text="❌ url is required")]

        tmp_dir = Path(tempfile.mkdtemp(prefix="mnemo_video_"))
        ...
        try:
            ...
            if is_youtube:
                ...
                if dl.returncode != 0 or not video_path.exists():
                    # yt-dlp sometimes appends an extension — find whatever it created
                    ...
                    if candidates:
                        ...
                        return [types.TextContent(
                            type="text",
                            text=f"❌ yt-dlp download failed:\n{dl.stderr[-1000:]}",
                        )]
            else:
                video_path = Path(url)
                if not video_path.exists():
                    return [types.TextContent(
                        type="text", text=f"❌ File not found: {url}"
                    )]
            ...
            frame_files = sorted(frames_dir.glob("frame_*.jpg"))[:max_frames]

            if not frame_files:
                return [types.TextContent(
                    type="text",
                    text="❌ No frames extracted — check that the video URL/path is valid.",
                )]

            contents: list = [types.TextContent(
                type="text",
                text=(
                    f"✅ {len(frame_files)} frames extracted "
                    f"(every {interval_sec}s, up to {max_frames} frames)\n"
                    f"Coverage: 0s → {(len(frame_files) - 1) * interval_sec}s\n"
                    "Frames follow — analyse each one:"
                ),
            )]

            for i, frame_file in enumerate(frame_files):
                ts = i * interval_sec
                contents.append(types.TextContent(type="text", text=f"[Frame {i + 1}  @  {ts}s]"))
                img_data = base64.standard_b64encode(frame_file.read_bytes()).decode()
            ...
            return [types.TextContent(
                type="text",
                text="❌ Timed out. Try a shorter video or increase interval_sec.",
            )]
        except Exception as exc:
    ...
    # ── get_video_transcript ──────────────────────────────────────────────────
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="mnemo_transcript_"))
        ...
        try:
            ...
            # ── Fast path: youtube-transcript-api (no download) ───────────────
            if video_id:
                try:
                    from youtube_transcript_api import YouTubeTranscriptApi
                    transcript = YouTubeTranscriptApi.get_transcript(video_id)
                    text = "\n".join(
                        f"[{int(t['start'])}s] {t['text']}" for t in transcript
                    )
                    return [types.TextContent(
                        type="text",
                        text=f"✅ Transcript ({len(transcript)} segments):\n\n{text}",
                    )]
                except ImportError:
            ...
            if vtt_files:
                ...
                return [types.TextContent(
                    type="text",
                    text=f"✅ Transcript (auto-captions, {len(lines)} lines):\n\n" + "\n".join(lines),
                )]
            ...
            return [types.TextContent(type="text", text="❌ Timed out fetching transcript.")]
        except Exception as exc:
            return [types.TextContent(type="text", text=f"❌ Error: {exc}")]
        finally:
            shutil.rmtree(tmp_dir, ignore_errors=True)
